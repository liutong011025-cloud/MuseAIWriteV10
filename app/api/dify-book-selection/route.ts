...
const DIFY_BOOK_SELECTION_API_KEY = process.env.DIFY_BOOK_SELECTION_API_KEY || process.env.DIFY_API_KEY || ''
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!DIFY_BOOK_SELECTION_API_KEY) {
      return NextResponse.json(
        { error: 'DIFY_BOOK_SELECTION_API_KEY not configured. Please set DIFY_BOOK_SELECTION_API_KEY or DIFY_API_KEY environment variable.' },
        { status: 500 }
      )
    }

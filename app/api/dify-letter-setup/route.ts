...
// Letter Setup 专用配置
// 支持从环境变量读取，如果没有设置则使用通用 DIFY_API_KEY
const DIFY_LETTER_API_KEY = process.env.DIFY_LETTER_API_KEY || process.env.DIFY_API_KEY || ''
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!DIFY_LETTER_API_KEY) {
      console.error('DIFY_LETTER_API_KEY not configured. Please set DIFY_LETTER_API_KEY or DIFY_API_KEY environment variable.')
      return NextResponse.json(
        { error: 'DIFY_API_KEY not configured. Please set DIFY_API_KEY environment variable.' },
        { status: 500 }
      )
    }
